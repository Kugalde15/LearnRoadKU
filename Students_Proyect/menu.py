    ...
    while True:
        ...
        try:
            ...
            if opt not in [1,2,3,4,5,6,7,8,9]:
                ...
                actions.add_student(students_data)
            elif opt == 2:
                actions.see_all_students(students_data)
            elif opt == 3:
                actions.top_3_average(students_data)
            elif opt == 4:
                actions.get_global_avg(students_data)
            elif opt == 5:
                data.export_students("student_test.csv", students_data, students_headers)
            elif opt == 6:
                students_data = data.import_students("student_test.csv")
            elif opt == 7:
                actions.del_student(students_data)
            elif opt == 8:
                actions.failed_students(students_data)
            else:
                exit()
